...
{  
   ...
   $exists = "SELECT * FROM `signup` WHERE user = '$user'";
   ...
   if($num>0){
     ...
     if($password==$cpassword){
        $sql = "INSERT INTO `signup` (`userid`, `user`, `password`, `date`) VALUES ('NULL','$user', '$password', current_timestamp())";
        $result = mysqli_query($conn,$sql);
        if($result){
            echo '<p class="success">Signup finished successfully</p>';
        }
    }
     else{
